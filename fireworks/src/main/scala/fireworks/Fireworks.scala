...
object Firework:
  ...
  /** @return
    ...
    *
    * If the firework is in the [[Done]] state, it stays in the same [[Done]]
    * state. Otherwise, the next state is computed by calling the operation
    * `next` on the underlying state type (see e.g., the operation `next` in the
    * class [[Waiting]]).
    *
    * Hint: choose what to do by pattern matching on the given `firework`. You
    * will have to use “typed patterns” to match on case classes and “literal
    * patterns” to match on case objects.
    */
  def next(firework: Firework): Firework = firework match
    case x @ Done     => x
    case x: Waiting   => x.next
    case x: Launched  => x.next
    case x: Exploding => x.next

end Firework

/** A firework waiting to be launched
  * @param countDown
  *   count-down before transitioning to the [[Launched]] state
  * @param startPosition
  *   location where this firework will be launched
  * @param numberOfParticles
  *   number of particles this firework will produce when exploding
  * @param particlesColor
  *   color of this firework
  */
case class Waiting(
    countDown: Int,
    startPosition: Point,
  ...
  /** @return
    *   The next state of this firework
    *
    ...
    *
    * Note that we use `def` here instead of `val` to avoid evaluating eagerly
    * all the next states.
    *
    * Hint: call the method `Launched.init(...)` (with the appropriate
    * arguments) to transition the firework to the [[Launched]] state.
    */
  def next: Firework =
...
object Waiting:
  ...
  /** @return
    *   A randomly initialized firework in a [[Waiting]] state
    */
  def init(): Waiting =
...
case class Launched(
  ...
  /** @return
    ...
    * the [[Launched]] state: it moves one step further in its [[direction]] and
    * decrements its [[countDown]] of one unit. Otherwise, it transitions to the
    * [[Exploding]] state.
    *
    * Hints: use the operation [[Motion.movePoint]] to compute the next position
    * of the firework, use the operation [[Exploding.init]] to transition the
    * firework to the [[Exploding]] state, and use the constant
...
object Launched:
  ...
  /** @return
    *   A firework in a [[Launched]] state, with a random direction
    * @param position
    *   position of the launch
    * @param numberOfParticles
    *   number of particle this firework will produce when exploding
    * @param particlesColor
    *   color of this firework
    */
  def init(
...
/** A firework that explodes
  * @param countDown
  *   count-down before the explosion is finished
  * @param particles
  *   Particles of this firework
  */
case class Exploding(countDown: Int, particles: Particles) extends Firework:
  ...
  /** @return
    ...
    *
    * As long as the [[countDown]] is greater than zero, the firework stays in
    * the [[Exploding]] state: it updates the state of its [[particles]] and
    * decrements its [[countDown]]. Otherwise, it transitions to the [[Done]]
    * state.
    *
    * Hint: use the operation [[Particles.next]] to compute the next state of
...
object Exploding:
  ...
  /** @return
    *   The initial state of a firework that is exploding
    * @param numberOfParticles
    *   number of particles in the explosion
    * @param direction
    *   direction of the firework when launched
    * @param position
  ...
  def init(
    ...
    val particles =
      List.fill(numberOfParticles)(Particle.init(direction, position, color))
...
/** A single particle in a firework explosion
  ...
  *   horizontal component of the particle speed
  * @param verticalSpeed
  *   vertical component of the particle speed
  * @param position
...
case class Particle(
  ...
  /** @return
    *   The next state of this particle
    *
    * Particles are subject to gravity and friction with air.
    *
    * Hint: use the operation [[Motion.drag]] to simulate the friction with the
    * surrounding air, and the constant [[Settings.gravity]] to simulate the
    * gravity.
    */
  def next: Particle =
...
case class Particles(value: List[Particle]):
  /** @return
    *   The next state of this group of particles
    */
...
object Particle:
  ...
  /** @return
    *   The initial state of a particle
    * @param initialDirection
    *   direction of the firework during its [[Launched]] phase
    * @param position
    *   position of the particle
    * @param color
    *   color of the particle
  ...
  def init(initialDirection: Angle, position: Point, color: Color): Particle =
    val angle = initialDirection + Angle(
...
object Motion:
  ...
  /** @return
    *   The next position of the given `point`, assuming that it moves towards
    *   the given `direction` at the given `speed`
    * @param point
  ...
  /** @return
    *   The given `speed` after applying friction to it
    */
